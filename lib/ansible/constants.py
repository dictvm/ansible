...
BECOME_METHODS            = ['sudo','su','pbrun','runas','sudo+su']
DEFAULT_BECOME_METHOD     = get_config(p, DEFAULTS, 'become_method', 'ANSIBLE_BECOME_METHOD','sudo' if DEFAULT_SUDO else 'su' if DEFAULT_SU else 'sudo' ).lower()
...
DEFAULT_BECOME_ASK_PASS   = get_config(p, DEFAULTS, 'become_ask_pass', 'ANSIBLE_BECOME_ASK_PASS',True if DEFAULT_ASK_SUDO_PASS else False, boolean=True)
